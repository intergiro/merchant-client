...
describe.skip("Client testing", () => {
	...
	beforeAll(async () => {
		privateClient = await model.Client.open(
			process.env.host ?? "backup",
			process.env.privateKey ?? "backup",
			"order",
			"merchant",
			"pspLog"
		)
		// TODO: Fix bug that only one client can exist: privateClient's key will be overwritten with publicClient's key.
		publicClient = await model.Client.open(
			process.env.host ?? "backup",
			process.env.publicKey ?? "backup",
			"card",
			"authorization"
		)
	})
